...
import { scheduleRecall, updateRecallEvent } from "@/actions/dogs";
import { getMonday, toDateString } from "@/lib/dates";
import type { RecallEvent } from "@/queries/dogs";
...
export function RecallBatchForm({
  trainers,
  mode,
  existingEvent,
}: {
  ...
  const [weekStartDate, setWeekStartDate] = useState(
    existingEvent
      ? existingEvent.weekStartDate
      : toDateString(getMonday(new Date()))
  );
  const [dogs, setDogs] = useState<DogRow[]>(
    existingEvent
      ? existingEvent.dogs.map((d) => ({
          id: d.id,
          name: d.name,
          trainerId: d.trainerId != null ? String(d.trainerId) : UNASSIGNED_TRAINER_VALUE,
        }))
      : []
  );
  ...
  function handleSubmit() {
    setError(null);
    const filledDogs = dogs.filter((d) => d.name.trim() !== "");
    if (mode === "create" && filledDogs.length === 0) {
      setError("Add at least one dog with a name.");
      return;
    }
    startTransition(async () => {
      try {
        if (mode === "create") {
          ...
          await updateRecallEvent({
            originalWeekStartDate: existingEvent.weekStartDate,
            weekStartDate,
            dogs: filledDogs.map((d) => ({
              id: d.id,
              name: d.name.trim(),
